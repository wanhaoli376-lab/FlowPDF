...
_SAVE_ARTIFACT = re.compile(
    r"(?:\.flowpdf-(?:save|export|story)-[0-9a-f]{32}\.tmp\.pdf"
    r"|\.flowpdf-project-[0-9a-f]{32}\.tmp)"
)


class SaveArtifactRegistry:
    """Remember exact save artifacts so a later launch can clean them."""
    ...
    def _validated_candidate(self, artifact: str | Path) -> Path:
        ...
        if _SAVE_ARTIFACT.fullmatch(candidate.name) is None:
            raise ValueError("不是 FlowPDF 已知的保存临时文件")
        try:
